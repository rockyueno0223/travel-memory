...
export const fetchMemories = async (userId: string): Promise<Memory[]> => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/memories/read?user_id=${userId}`, {
...
export const addMemory = async (memoryData: Partial<Memory>): Promise<Memory | null> => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/memories/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export const updateMemory = async (memoryData: Partial<Omit<Memory, "id">> & { id: number }): Promise<Memory | null> => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/memories/update`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(memoryData),
    });
...
export const deleteMemory = async (memoryId: number): Promise<boolean> => {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/memories/delete?memory_id=${memoryId}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
    });
